...
<body>
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<c:choose>
			<c:when test="${not empty templateparentList}">
				<c:forEach items="${templateparentList}" var="template" varStatus="vs">
				<tr id="tr${template.cms_id }">
				<td class="center">${vs.index+1}</td>
				<td class='center'><i class="${template.cms_icon }">&nbsp;</i>${template.cms_name }&nbsp;
					<c:if test="${template.cms_type == '1' }">
				...
				<td>${template.template_url == '#'? '': template.template_url}</td>
				<td class='center'>${template.cms_order }</td>
				<td style="width: 25%;">
				<a class='btn btn-mini btn-warning' onclick="openClose('${template.cms_id }',this,${vs.index })" >展开</a>
				<a class='btn btn-mini btn-purple' title="图标" onclick="editTb('${template.cms_id }')" ><i class='icon-picture'></i></a>
				<a class='btn btn-mini btn-info' title="编辑" onclick="editmenu('${template.cms_id }')" ><i class='icon-edit'></i></a>
				<a class='btn btn-mini btn-danger' title="删除"  onclick="delmenu('${template.cms_id }',true)"><i class='icon-trash'></i></a>
				</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
